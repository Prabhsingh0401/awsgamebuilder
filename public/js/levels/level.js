(function() {
  ...
  Level.prototype.putGoomba = function(x, y) {
    ...
    this.enemies.push(new Mario.Goomba([16*(x+2), 16*y], this.goombaSprite() ));
  };

  Level.prototype.putKoopa = function(x, y) {
    this.enemies.push(new Mario.Koopa([16*x, 16*y], this.koopaSprite(), false));
    this.enemies.push(new Mario.Koopa([16*(x+3), 16*y], this.koopaSprite(), false));
  };
  ...
  Level.prototype.putBrick = function(x, y, item) {
    if (!this.blocks[y]) {
        console.error('Invalid y coordinate for block:', y);
        return;
    }
